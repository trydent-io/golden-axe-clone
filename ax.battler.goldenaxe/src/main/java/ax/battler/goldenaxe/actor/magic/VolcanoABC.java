...
public class VolcanoABC extends Magic {
      ...
      for (int col = 0; col < MAX_EXPLOSIONS_X; col++) {
        ...
        if (explosion.isDestroyed()
          ...
          switch (type) {
            ...
            case VOLCANO_B -> size = (Util.random(2) == 0) ? "small" : "medium";
  ...
  public enum Type {
    VOLCANO_A("a", 20, 2, 2), VOLCANO_B("b", 40, 4, 4), VOLCANO_C("c", 60, 6, 6);
    ...
    Type(String letter, int damage, int score, int consumedPotions) {
      this.letter = letter;
      this.damage = damage;
      this.score = score;
