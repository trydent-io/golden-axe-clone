...
import ax.battler.goldenaxe.actor.Villager;
import ax.battler.goldenaxe.actor.Water;
import ax.battler.goldenaxe.scene.Stage;

import java.util.UUID;

import static ax.battler.goldenaxe.infra.Command.Type.ANIMATOR;
import static ax.battler.goldenaxe.infra.Command.Type.SPAWN;
...
public class Command extends Actor {
  ...
  public Command(Stage stage, String[] parameters) {
    ...
    // spawn ax.battler.goldenaxe.actor
    if (type == SPAWN) {
  ...
  private Actor getSpawnActor(String[] parameters) {
    ...
    switch (parameters[3]) {
      ...
      case "chicken_leg", "dragon_blue", "dragon_red"
        ...
        , "death_adder" -> actor = new Enemy(stage, parameters[3], parameters);
      case "decoration" -> actor = new Decoration(stage, parameters);
      case "water" -> actor = new Water(stage);
      case "villager" -> actor = new Villager(stage, parameters);
      case "thief" -> {
        String thiefId = "thief_" + UUID.randomUUID();
        actor = new Thief(stage, thiefId, parameters);
      }
      default -> {
        throw new RuntimeException("invalid ax.battler.goldenaxe.actor !");
      }
    }
    return actor;
  }
