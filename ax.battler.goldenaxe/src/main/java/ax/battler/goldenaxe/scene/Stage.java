...
import ax.battler.goldenaxe.actor.Decoration;
...
import ax.battler.goldenaxe.infra.BackgroundTransition;
...
import ax.battler.goldenaxe.infra.Go;
import ax.battler.goldenaxe.infra.GoldenAxeGame;
import ax.battler.goldenaxe.infra.Input;
import ax.battler.goldenaxe.infra.MagicBackgroundTransition;
...
import static ax.battler.goldenaxe.infra.Collider.Type.BODY;
import static ax.battler.goldenaxe.infra.Collider.Type.FIRE;
import static ax.battler.goldenaxe.infra.Collider.Type.TOUCH;
...
import static ax.battler.goldenaxe.infra.Settings.SHOW_DEBUG_CAMERA_INFO;
import static ax.battler.goldenaxe.infra.Settings.SHOW_DEBUG_ENEMIES_COUNT;
import static ax.battler.goldenaxe.infra.Settings.SHOW_DEBUG_PLAYER_1_INFO;
import static ax.battler.goldenaxe.infra.Settings.SHOW_DEBUG_PLAYER_2_INFO;
import static ax.battler.goldenaxe.infra.Settings.VALID_COLLISION_WZ_DIF;
...
public class Stage extends State<GoldenAxeGame, SceneManager> {
  ...
  public Stage(SceneManager manager, GoldenAxeGame game) {
    ...
    for (int layer = 0; layer < MAX_BACKGROUND_LAYERS; layer++) {
      switch (layer) {
        case MAGIC_LAYER -> backgrounds[layer] = new MagicBackgroundTransition();
  ...
  public void draw(Graphics2D g) {
    ...
    if (SHOW_DEBUG_CAMERA_INFO) {
      String cameraInfo = String.format("camera x=%d y=%d min=%d max=%d"
        ...
        , Camera.getMinY((int) (Camera.getX() + CANVAS_WIDTH))
        , Camera.getMaxY((int) (Camera.getX() + CANVAS_WIDTH)));
    ...
    if (SHOW_DEBUG_PLAYER_1_INFO && GoldenAxeGame.player1 != null) {
      String player1Info = String.format("p1 x=%d y=%d z=%d sc=%d"
        ...
        , GoldenAxeGame.player1.getScore());
      g.setColor(Color.WHITE);
    ...
    if (SHOW_DEBUG_PLAYER_2_INFO && GoldenAxeGame.player2 != null) {
      String player2Info = String.format("p2 x=%d y=%d z=%d sc=%d"
        ...
        , GoldenAxeGame.player2.getScore());
      g.setColor(Color.WHITE);
